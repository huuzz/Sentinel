...
export interface DashboardSummary { total_events:number; alerts_today:number; alerts_by_severity:{name:string;count:number}[]; common_event_types:{name:string;count:number}[]; event_volume:{timestamp:string;count:number}[]; recent_alerts:Alert[]; }
